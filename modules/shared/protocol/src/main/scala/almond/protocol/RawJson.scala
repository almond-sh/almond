...
final class RawJson(val value: Array[Byte]) {
  ...
  override def toString: String =
    Try(new String(value, StandardCharsets.UTF_8)) match {
      case Success(str)  => str
      case f: Failure[_] => f.toString
    }
}

object RawJson {

  def apply(value: Array[Byte]): RawJson =
    new RawJson(value)
