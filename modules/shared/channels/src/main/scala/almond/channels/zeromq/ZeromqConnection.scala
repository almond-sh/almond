...
final class ZeromqConnection(
  ...
  private val heartBeatPortPromise =
    if (bind && bindToRandomPorts && new URI(params.heartbeatUri).getPort <= 0)
      Some((Promise[Int](), ZeromqSocketImpl.removePort(params.heartbeatUri)))
    else
      None
  ...
  val open: IO[Map[Option[Channel], Int]] = {
    ...
    val maybeHeartBeatPort = heartBeatPortPromise match {
      case Some((promise, _)) =>
