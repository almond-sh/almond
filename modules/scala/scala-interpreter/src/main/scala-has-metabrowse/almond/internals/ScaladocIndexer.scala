// from https://github.com/scalameta/metals/blob/f4ac29634cdf4049708fc40d4c0fc98ffe343ae5/mtags/src/main/scala-2/scala/meta/internal/metals/ScaladocIndexer.scala
// with minor adjustment to work around InternalTree private method origin
...
class ScaladocIndexer(
  ...
  override def visitOccurrence(
    ...
    val docstring = currentTree.pos match {
      case inputs.Position.None => ""
      case inputs.Position.Range(_, start, _) =>
        val leadingDocstring =
          ScaladocIndexer.findLeadingDocstring(
            source.tokens,
            source.tokens.indexWhere(_.start == start) - 1
          )
